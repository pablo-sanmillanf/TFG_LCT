...
class TextLabel(QLabel):
    ...
    def text_selected(self, text_index: int) -> None:
        ...
        for i in range(len(text_list)):
            if i == text_index:
                aux_document = QTextDocument()
                aux_document.setDefaultFont(self.font())
                aux_document.setTextWidth(self.width())

                offset = aux_document.size().height() * 1.2

                aux_document.setHtml(text + "</p>")
                self.scroll_updated.emit(int(aux_document.size().height() - offset))
